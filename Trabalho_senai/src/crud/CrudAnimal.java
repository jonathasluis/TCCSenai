...
public class CrudAnimal {
	...
	public static ResultSet selecionaAnimais(Animal animal) {
		...
		String sql = "SELECT * FROM animais where idfazenda = ? and idanimal>1 order by nomeLote";
		try {
			PreparedStatement stmt = Conexao.conexao.prepareStatement(sql);
	...
	public static ResultSet procuraAnimal(String letra,Animal animal) {
		ResultSet tabela = null;
		String sql = "SELECT * FROM animais WHERE nomelote LIKE ? and idfazenda = ? and idanimal>1  order by nomeLote";
		try {
			PreparedStatement stmt = Conexao.conexao.prepareStatement(sql);
